...
namespace MqttORM.Bus
{
    ...
    public class MqttBus : IMqttBus
    {
        ...
        {
            var topic = ResolveTopic<T>(parameters);
            ...
            object? parameters = null)
        {
            ...
            var topic = parameters == null
                ? attr.Template
                : attr.Resolve(parameters as TopicValues ?? new TopicValues().Convert(
                    parameters.GetType()
        ...
        /// <summary>
        /// Resolves a topic template for type T with optional parameters.
        /// </summary>
        private static string ResolveTopic<T>(object? parameters = null)
        {
            var attr = typeof(T).GetCustomAttribute<MqttTopicAttribute>()
                       ?? throw new InvalidOperationException($"Type '{typeof(T).Name}' lacks [MqttTopic].");
            return parameters == null
                ? attr.Template
                : attr.Resolve(parameters as TopicValues
                    ?? new TopicValues().Convert(
                        parameters.GetType()
                                  .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                                  .ToDictionary(p => p.Name, p => p.GetValue(parameters)?.ToString()!)));
        }
